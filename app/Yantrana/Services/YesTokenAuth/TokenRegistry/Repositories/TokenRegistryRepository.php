...
class TokenRegistryRepository extends BaseRepository
{
     ...
     * @return mixed
     *-----------------------------------------------------------------------*/
    public function delete($idOrUid, $onlyOld = false)
    {
        if ($onlyOld) {
            $count = 0;
            $records = TokenRegistryModel::where('predecessor_token_id', $idOrUid)->latest()->get();
        ...
        return TokenRegistryModel::where('predecessor_token_id', $idOrUid)->delete();
    }
    ...
    /**
     * Delete token by session
     *
     * @return Eloquent collection object
     *-----------------------------------------------------------------------*/
    public function deleteTokenSession($sessionTokenId)
    {
        return TokenRegistryModel::where('predecessor_token_id', $sessionTokenId)->delete();
    }
}
